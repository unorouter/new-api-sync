...
// prettier-ignore
const ProviderCommonProps = { name: str, testModelTypes: Opt(T.Array(ModelTypeEnum)), enabledModels: Opt(T.Array(EnabledModelEntrySchema)), priceAdjustment: Opt(PriceAdjustmentSchema), perUpstreamConcurrency: Opt(T.Integer({ minimum: 1, maximum: 1000 })), perUpstreamRpm: Opt(T.Number({ exclusiveMinimum: 0, maximum: 6000 })), authenticityPassTtlHours: Opt(T.Number({ exclusiveMinimum: 0, maximum: 720 })), channelLimits: Opt(T.Object({ maxConcurrency: Opt(T.Integer({ minimum: 1, maximum: 10000 })), maxRps: Opt(T.Number({ exclusiveMinimum: 0, maximum: 10000 })) })), autoTestIntervalMinutes: Opt(T.Integer({ minimum: 1, maximum: 10080 })), autoTestIntervalMaxMinutes: Opt(T.Integer({ minimum: 1, maximum: 10080 })), headerOverride: Opt(T.Record(str, str)), forceUpstreamStream: Opt(T.Union([T.Boolean(), T.Record(T.String(), T.Boolean())])) } as const;
...
// prettier-ignore
const OpenRouterProviderSchema = T.Object({ type: T.Literal("openrouter"), ...ProviderCommonProps, baseUrl: Opt(uri), apiKey: str, models: Opt(T.Array(str)), ratio: Opt(T.Number({ minimum: 0 })), acceptRateLimited: Opt(T.Boolean()), hostsPerModel: Opt(T.Record(T.String(), T.Integer({ minimum: 1 }))), allowQuantizations: Opt(T.Record(T.String(), T.Array(T.String()))), managementKey: Opt(str), keyDailyLimitUsd: Opt(T.Union([T.Number({ minimum: 0 }), T.Record(T.String(), T.Number({ minimum: 0 }))])), keyExpiryDays: Opt(T.Integer({ minimum: 1 })), requireKeyStore: Opt(T.Boolean()) });
