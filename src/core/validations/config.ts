...
const A7ProviderSchema = T.Object({ type: T.Literal("a7"), ...ProviderCommonProps, baseUrl: uri, systemAccessToken: str, userId: T.Integer({ minimum: 1 }), profitMultiple: Opt(T.Number({ minimum: 1 })), maxSellFraction: Opt(T.Number({ exclusiveMinimum: 0, maximum: 1 })), hostsPerModel: Opt(T.Record(T.String(), T.Integer({ minimum: 1 }))), minSuccessRate: Opt(T.Integer({ minimum: 0, maximum: 10000 })), guaranteedOnly: Opt(T.Boolean()), acceptRateLimited: Opt(T.Boolean()) });
