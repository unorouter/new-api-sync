...
import { processIhProvider } from "@core/vendors/ih/provider";
import { processSiProvider } from "@core/vendors/si/provider";
  ...
  const settled = await Promise.all(
    sorted.map((provider) =>
      timingTrack(`provider:${provider.name}`, async () => {
        ...
        if (provider.type === "ih")
          return processIhProvider(provider, config, ctx);
        if (provider.type === "si")
          return processSiProvider(provider, config, ctx);
