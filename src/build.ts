...
// string + content type, plus config.example.yml as a raw string. Both the
// js bundle and native binaries import this and serve from memory — no disk
// reads, no separate public/ dir required.
const writeAssetManifest = (assets: string[]) => {
  ...
  lines.push("];");
  lines.push("");
  const example = readFileSync("config.example.yml", "utf8");
  lines.push(
    `export const embeddedConfigExample = ${JSON.stringify(example)};`,
  );
  lines.push("");
