...
src/
  ...
  core/                   provider-agnostic engine
    ...
    vendors/              per-provider adapters: newapi (reference), a7, openrouter, nvidia, comfyui,
...
2. **Discover + test** per provider in TYPE_ORDER (newapi, nvidia, openrouter, a7). Each
...
`metadata` runs `runProviderPipeline({dryRun:true})` over ONLY the `newapi` and `a7` providers (the
...
- **a7 `minSellFraction` is a retail FLOOR, `maxSellFraction` a merchant CUT.** Both are
  ...
  `vendors/a7/provider.ts`, logged as `[a7] floor ...`). Ceiling wins if they overlap.
...
The a7.com marketplace provider (type `a7`) is NAMED `a7` in config and in the gateway since 2026-09-06 (channel
...
- a7 PAUSES a pin whenever its merchant reprices (`paused_price_changed`);
  ...
  (`acceptPriceNotices` in vendors/a7/pins.ts).
